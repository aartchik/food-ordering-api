...
[Публичный API](https://food-ordering-aartchik.duckdns.org/healthz)
...
5. Если меню изменилось после наполнения корзины, получает `409 Conflict` со списком изменившихся позиций и их актуальными значениями.
6. Может безопасно повторить запрос: тот же ключ и те же данные возвращают созданный заказ, а другие данные с тем же ключом приводят к ответу `409 Conflict`.
7. Получает актуальный статус заказа.

API предотвращает оформление пустой корзины, смешивание позиций разных заведений и заказ недоступной позиции. Названия и цены копируются в корзину. Перед оформлением они сравниваются с актуальным меню, поэтому изменение цены, названия или доступности требует подтверждения пользователя. После оформления данные заказа больше не зависят от изменений меню.
